...
const DisplayAllPosts = (props) => {
  return(
    ...
    <div key={post.id}>
      <RIETextArea
        value={post.title}
        change={(value) => props.editPostTitle({
          id: post.id,
        newValue: value
      })}
      propName='title'
      ...
      {props.showBody && (<RIETextArea
        value={post.body}
        change={(value) => props.editPostBody({
        id: post.id,
        newValue: value
      })}
      propName='body'
      validate={_.isString}
      className='col-lg-12' />)}
      <p className="col-md-12"> Comments : {props.getNumOfComments(post.id)}</p>
      <p className="col-md-12"> Votes:{post.voteScore} </p>
      <p className="col-md-12"> Author: {post.author} </p>
      <p className="col-md-12"> Time: {post.timestamp} </p>
      {props.showMoreLink && (<Link to={`/${post.category}/${post.id}`}>
          <p className="col-md-12"> More </p>
      </Link>)}
...
class Posts extends Component {
  ...
  editPostTitle = ({id,newValue}) => {
    API.updatePost(id,{'timestamp':Date.now(),'title':newValue.title}).then((data) =>
    this.props.editPost({
      id: data.id,
      attribute: 'TITLE',
      value: data.title,
